$axure.internal(function($ax) {
    ...
    var _initializePageFragment = function(pageFragment, objIdToObject) {
        var objectArrayHelper = function(objects, parent, packageId) {
            for(var i = 0; i < objects.length; i++) {
                diagramObjectHelper(objects[i], parent, packageId);
            }
        };

        var diagramObjectHelper = function(diagramObject, parent, packageId) {
            ...
            if(diagramObject.diagrams) { //dynamic panel
                for(var i = 0; i < diagramObject.diagrams.length; i++) {
                    ...
                    objectArrayHelper(diagram.objects, diagram, packageId);
                }
            } else if($ax.public.fn.IsLayer(diagramObject.type)) {
                var layerObjs = diagramObject.objs;
                objectArrayHelper(layerObjs, parent, packageId);
            }
            // if the rdo object has children, it means it's overridden
            // use rdo id as packageId in this case
            if(diagramObject.objects) objectArrayHelper(diagramObject.objects, diagramObject, ($ax.public.fn.IsReferenceDiagramObject(diagramObject.type) && diagramObject.id) || packageId);
        };
        objectArrayHelper(pageFragment.diagram.objects, pageFragment.diagram, pageFragment.packageId);
    };
